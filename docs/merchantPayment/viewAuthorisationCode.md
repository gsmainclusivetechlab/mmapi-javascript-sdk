...
Here, if only a single account ID is provided, `createAuthorisationCode` creates a GET request to /accounts/{identifierType}/{identifier}/authorisationcodes/{authorisationCode}

> `This endpoint returns a specific Authorisation Code linked to an account.`

Here, if only a single account ID is provided, `createAuthorisationCode` creates a GET request to /accounts/{RequestorAccountIdentifiers}/authorisationcodes/{authorisationCode} depending on the number of account IDs provided.
