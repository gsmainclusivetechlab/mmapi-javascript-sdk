...
Here, if only a single account ID is provided, `viewAuthorisationCode` creates a GET request to /accounts/{identifierType}/{identifier}/authorisationcodes/{authorisationCode}

> `This endpoint returns a specific Authorisation Code linked to an account where one identifier suffices to uniquely identify an account.`

Here, if Multiple account IDs are provided, `viewAuthorisationCode` creates a GET request to /accounts/{accountId}/authorisationcodes/{authorisationCode}

> `This endpoint returns a specific Authorisation Code linked to an account where a single identifier is not sufficient to identify an account.`
